...
// ==============================
// DELAY TYPING FUNCTIONALITY - FULL COMPLETE IMPLEMENTATION
// ==============================
...
function showTypingAction($chat_id, $duration_ms = 1500) {
    // Bas typing action dikhata hai (without sending message)
    $typing_data = [
        'chat_id' => $chat_id,
        'action' => 'typing'
    ];
...
function sendTypingAndMessage($chat_id, $text, $options = []) {
    // Typing action + message combined function
    $delay = $options['delay'] ?? 1000;
    $parse_mode = $options['parse_mode'] ?? null;
    $reply_markup = $options['reply_markup'] ?? null;
    
    showTypingAction($chat_id, $delay);
    return sendMessage($chat_id, $text, $reply_markup, $parse_mode);
}
...
// ALL CHANNELS DEFINED HERE - UPDATED WITH CORRECT IDs
define('MAIN_CHANNEL', '@EntertainmentTadka786');  // Main channel username
define('MAIN_CHANNEL_ID', '-1003181705395');  // Main channel ID - UPDATED
define('THEATER_CHANNEL', '@threater_print_movies');  // Theater channel username
define('THEATER_CHANNEL_ID', '-1002831605258');  // Theater channel ID - UPDATED
define('BACKUP_CHANNEL_USERNAME', '@ETBackup');  // Backup channel username
define('BACKUP_CHANNEL_ID', '-1002964109368');  // Backup Channel of Movies And Webseries - UPDATED
define('BACKUP_CHANNEL_2_USERNAME', '@ETBackup');  // Same as above
define('BACKUP_CHANNEL_2_ID', '-1002964109368');  // Same as above - UPDATED
define('REQUEST_CHANNEL', '@EntertainmentTadka7860');  // Request channel username
define('REQUEST_CHANNEL_ID', '-1003083386043');  // Request Group ID - UPDATED
...
define('QUICKADD_FORMAT', "🎬 <b>Quick Add Format:</b>\n\n" .
    ...
    "• <code>/quickadd Animal (2023),54321,-1002964109368</code>\n\n" .
    "<b>Supported Channels:</b>\n" .
    "• @EntertainmentTadka786 (Main Channel)\n" .
    "• @threater_print_movies (Theater Prints)\n" .
    "• @ETBackup (Backup Channel)\n" .
    "• -1002964109368 (Backup Channel ID)\n\n" .
...
// ==============================
// CHANNEL MAPPING FUNCTIONS - UPDATED
// ==============================
...
function get_channel_display_name($channel_type) {
    ...
    $names = [
        'main' => '🍿 Movies and Webseries',
        'theater' => '🎭 Threater_Print_Movies',
        'backup' => '🔒 Backup Channel of Movies And Webseries',
        'other' => '📢 Other Channel'
    ];
...
// ==============================
// CSV MANAGEMENT FUNCTIONS - FORMAT LOCKED: movie_name,message_id
// ==============================
...
function forwardMessage($chat_id, $from_chat_id, $message_id) {
    // Message forward karta hai - FORWARDING ON KAR DIYA
...
function deliver_item_to_chat($chat_id, $item) {
    // Movie user ko deliver karta hai - WITH FORWARDING
    ...
    if (!empty($item['message_id_raw'])) {
        ...
        if (is_numeric($message_id_clean) && $message_id_clean > 0) {
            ...
            if ($result && $result['ok']) {
                ...
                // Fallback to copy
                $fallback_result = json_decode(copyMessage($chat_id, $source_channel, $message_id_clean), true);
...
function handle_command($chat_id, $user_id, $command, $params = []) {
    ...
    switch ($command) {
        ...
        case '/start':
            ...
            sendTypingAndMessage($chat_id, $welcome, ['parse_mode' => 'HTML', 'reply_markup' => $keyboard, 'delay' => 1500]);
        ...
        case '/commands':
            ...
            sendTypingAndMessage($chat_id, $help, ['parse_mode' => 'HTML', 'reply_markup' => $keyboard, 'delay' => 1000]);
            break;
        ...
        case '/find':
            ...
            if (empty($movie_name)) {
                sendTypingAndMessage($chat_id, "❌ Usage: <code>/search movie_name</code>\nExample: <code>/search kgf 2</code>", ['parse_mode' => 'HTML', 'delay' => 500]);
                return;
            }
        ...
        case '/requestmovie':
            $movie_name = implode(' ', $params);
            if (empty($movie_name)) {
                sendTypingAndMessage($chat_id, "❌ Usage: <code>/request movie_name</code>\nExample: <code>/request Animal Park</code>", ['parse_mode' => 'HTML', 'delay' => 500]);
                return;
            }
            $lang = detect_language($movie_name);
        ...
        case '/botstats':
            if ($user_id == ADMIN_ID) {
                ...
                sendTypingAndMessage($chat_id, "❌ Access denied. Admin only command.", ['delay' => 500]);
            }
            break;
        ...
        case '/broadcast':
            if ($user_id == ADMIN_ID) {
                ...
                if (empty($message)) {
                    sendTypingAndMessage($chat_id, "❌ Usage: <code>/broadcast your_message</code>", ['parse_mode' => 'HTML', 'delay' => 500]);
                    return;
                }
                showTypingAction($chat_id, 1000);
        ...
        case '/sendalert':
            if ($user_id == ADMIN_ID) {
                ...
                if (empty($alert_message)) {
                    sendTypingAndMessage($chat_id, "❌ Usage: <code>/sendalert your_alert</code>", ['parse_mode' => 'HTML', 'delay' => 500]);
                    return;
                }
                showTypingAction($chat_id, 800);
        ...
        case '/ping':
        case '/status':
            sendTypingAndMessage($chat_id, "🏓 <b>Bot Status:</b> ✅ Online\n⏰ <b>Server Time:</b> " . date('Y-m-d H:i:s'), ['parse_mode' => 'HTML', 'delay' => 500]);
            break;
        ...
        case '/reportbug':
            ...
            if (empty($bug_report)) {
                sendTypingAndMessage($chat_id, "❌ Usage: <code>/report bug_description</code>", ['parse_mode' => 'HTML', 'delay' => 500]);
                return;
            }
            showTypingAction($chat_id, 800);
        ...
        case '/feedback':
            ...
            if (empty($feedback)) {
                sendTypingAndMessage($chat_id, "❌ Usage: <code>/feedback your_feedback</code>", ['parse_mode' => 'HTML', 'delay' => 500]);
                return;
            }
            showTypingAction($chat_id, 800);
        ...
        default:
            sendTypingAndMessage($chat_id, "❌ Unknown command. Use <code>/help</code> to see all available commands.", ['parse_mode' => 'HTML', 'delay' => 500]);
    }
}
...
if (isset($_GET['test_save'])) {
    ...
    echo "✅ All 8 movies manually save ho gayi!<br>";
...
if (php_sapi_name() === 'cli' || isset($_GET['setwebhook'])) {
    ...
    if ($bot_info && isset($bot_info['ok']) && $bot_info['ok']) {
        ...
        echo "<p>Request Channel: " . REQUEST_CHANNEL . " (" . REQUEST_CHANNEL_ID . ")</p>";
    }
...
if (!isset($update) || !$update) {
    ...
    echo "<p><strong>Request Channel:</strong> " . REQUEST_CHANNEL . " (" . REQUEST_CHANNEL_ID . ")</p>";
    ...
    echo "<li>✅ ALL CHANNELS SUPPORT - 3 channels integrated</li>";
    ...
    echo "<li>✅ <b>FORWARDING ON</b> - Using forwardMessage for delivery</li>";
    echo "<li>✅ <b>DELAY TYPING</b> - Full complete implementation</li>";
