...
define('QUICKADD_FORMAT', "🎬 <b>Quick Add Format:</b>\n\n" .
    "<code>/quickadd movie_name (year),message_id,channel_username</code>\n\n" .
    "<b>Examples:</b>\n" .
    ...
    "• <code>/quickadd Animal (2023),54321,-1001234567890</code>\n\n" .
        ...
        while (($row = fgetcsv($handle)) !== FALSE) {
            if (count($row) >= 3 && (!empty(trim($row[0])))) {
                ...
                $entry = [
                    ...
                    'source_channel' => $channel_id ?: ($channel_type == 'theater' ? THEATER_CHANNEL_ID : CHANNEL_ID)
                ];
...
function quick_add_movie($chat_id, $user_id, $input_data) {
    ...
    if (strpos($channel_info, '@') === 0) {
        ...
        // Channel username se channel type determine karo
        if ($channel_username == '@' . ltrim(THEATER_CHANNEL, '@')) {
            $channel_type = 'theater';
            $channel_id = THEATER_CHANNEL_ID;
        } elseif ($channel_username == '@' . ltrim(MAIN_CHANNEL, '@')) {
            $channel_type = 'main';
            $channel_id = CHANNEL_ID;
        } else {
            // Other channel - try to get ID
            $channel_type = 'other';
        }
        ...
        // Channel ID se channel type determine karo
    ...
    if ($handle !== FALSE) {
        ...
        $success_msg .= "🎭 <b>Channel Type:</b> " . ucfirst($channel_type) . "\n";
        ...
        $channel_id_clean = str_replace('-100', '', $channel_id);
        $success_msg .= "https://t.me/c/$channel_id_clean/$message_id\n\n";
        
        $success_msg .= "📊 Movie database updated successfully!";
        
        sendMessageWithDelay($chat_id, $success_msg, null, 'HTML', 1500);
        bot_log("Movie quick added by $user_id: $movie_name (ID: $message_id) to $channel_type channel");
...
function admin_stats($chat_id) {
    ...
    $msg .= "\n📦 Recent Uploads:\n";
    foreach ($recent as $r) {
        $channel_icon = (isset($r['channel_type']) && $r['channel_type'] == 'theater') ? '🎭' : '🍿';
...
if (!isset($update) || !$update) {
    ...
    echo "<li>✅ QUICK ADD MOVIES - Admin can add movies quickly</li>";
