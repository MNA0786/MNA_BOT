...
if (!CHANNEL_ID || !BACKUP_CHANNEL_ID || !THEATER_CHANNEL_ID) {
    die("❌ Essential channel IDs missing.");
}
...
function get_channel_icon($channel_type) {
    global $CHANNEL_CONFIG;
    return $CHANNEL_CONFIG[$channel_type]['icon'] ?? '🎬';
}
...
function append_movie($movie_name, $message_id, $channel_input = 'main', $quality = 'Unknown', $size = 'Unknown', $language = 'Hindi') {
    ...
    if ($stmt->execute()) {
        ...
        if (!empty($waiting_users[$movie_lower])) {
            ...
            $notification_msg .= "📢 Join: {$channel_info['username']} to download\n";
            $notification_msg .= "🔔 " . count($waiting_users[$movie_lower]) . " users were waiting!\n\n";
...
function deliver_item_to_chat($chat_id, $item, $source_type = null) {
    ...
    $text .= "📅 Date: " . htmlspecialchars($item['added_date'] ?? 'N/A') . "\n\n";
...
function handle_admin_command($chat_id, $user_id, $command, $params = []) {
    if ($user_id != ADMIN_ID) {
        return sendMessage($chat_id, "❌ Access denied. Admin only command.");
    }
    ...
    switch ($base_command) {
        ...
        default:
            sendMessage($chat_id, "❌ Unknown admin command. Use /adminhelp");
    }
}
...
function admin_add_movie($chat_id, $params) {
    if (count($params) < 3) {
        ...
        $help .= "🎯 <b>Channels:</b> main, theater, backup, request";
    ...
    if ($success) {
        ...
        $response = "✅ <b>Movie Added!</b>\n\n";
        $response .= "🎬 <b>Movie:</b> $movie_name\n";
        $response .= "🆔 <b>Message ID:</b> $message_id\n";
        $response .= "📢 <b>Channel:</b> {$channel_info['username']}\n";
        $response .= "🔗 <b>Direct Link:</b> $direct_link";
        
        $keyboard = [
            'inline_keyboard' => [
                [
                    ['text' => '🔗 Open Link', 'url' => $direct_link],
                    ['text' => '📋 List All', 'callback_data' => 'admin_list_1']
                ]
            ]
        ];
        ...
        sendMessage($chat_id, "❌ Failed to add movie. May already exist.");
    }
}
...
function admin_list_movies($chat_id, $params) {
    ...
    $message = "📋 <b>Movies (Page $page of $total_pages)</b>\n\n";
    ...
    $nav_row = [];
    if ($page > 1) {
        $nav_row[] = ['text' => '⬅️ Previous', 'callback_data' => 'admin_list_' . ($page-1) . ($channel_filter ? '_' . $channel_filter : '')];
    }
    
    if ($page < $total_pages) {
        $nav_row[] = ['text' => 'Next ➡️', 'callback_data' => 'admin_list_' . ($page+1) . ($channel_filter ? '_' . $channel_filter : '')];
    }
...
function admin_find_movie($chat_id, $params) {
    ...
    if (empty($movies)) {
        sendMessage($chat_id, "❌ No movies found for '$search_query'");
        return;
    }
...
function admin_show_stats($chat_id) {
    ...
    $message .= "🎬 <b>Movies:</b> $total_movies total\n";
    $message .= "   📅 Today: $today_movies added\n\n";
    $message .= "👥 <b>Users:</b> $total_users total\n";
    $message .= "   📅 Active (24h): $active_users\n\n";
    $message .= "🔍 <b>Searches:</b> $total_searches total\n";
    $message .= "📥 <b>Downloads:</b> $total_downloads total\n\n";
...
function admin_dashboard($chat_id) {
    ...
    $message = "🛠️ <b>Admin Dashboard</b>\n\n";
    $message .= "📊 <b>Quick Stats:</b>\n";
    $message .= "• 🎬 Movies: $total_movies total\n";
    $message .= "• 📅 Today: $today_movies added\n";
    $message .= "• 👥 Users: $total_users total\n";
    $message .= "• 📅 Active (24h): $active_users\n\n";
    ...
    $keyboard = [
        'inline_keyboard' => [
            [
                ['text' => '➕ Add Movie', 'callback_data' => 'admin_add_quick'],
                ['text' => '📋 List Movies', 'callback_data' => 'admin_list_1']
            ],
            [
                ['text' => '🔍 Find Movie', 'callback_data' => 'admin_search_menu'],
...
function advanced_search($chat_id, $query, $user_id = null) {
    ...
    if ($invalid_count > 0 && ($invalid_count / $total_words) > 0.5) {
        ...
        $help_msg .= "🔍 Examples:\n";
        $help_msg .= "• kgf\n• pushpa\n• avengers\n• hindi movie\n\n";
        $help_msg .= "❌ Technical queries like 'vlc', 'audio track' are not movie names.\n\n";
    ...
    if (!empty($found)) {
        ...
        foreach ($found as $movie => $data) {
            ...
            $msg .= "   📊 " . ($movie_data['quality'] ?? 'Unknown') . " | 🗣️ " . ($movie_data['language'] ?? 'Hindi') . "\n\n";
            $i++;
            if ($i > 5) break;
        }
        ...
        sendMessage($chat_id, 
            "😔 '$query' available nahi hai!\n\n" .
            "📝 Request kar sakte hain: " . REQUEST_CHANNEL . "\n\n" .
            "🔔 Add hone par auto-notification milega!"
        );
...
function handle_command($chat_id, $user_id, $command, $params = []) {
    ...
    switch ($command) {
        case '/start':
            ...
            $welcome .= "💬 <b>Need help?</b> Just type movie name!";
        ...
        case '/theater':
            sendMessage($chat_id, 
                ...
                "🎬 Theater Channel: " . THEATER_CHANNEL
            );
            break;
        ...
        default:
            sendMessage($chat_id, 
                "❌ Unknown command.\n\n" .
                "💡 Just type movie name to search!\n" .
...
function totalupload_controller($chat_id, $page = 1, $filters = [], $session_id = null) {
    ...
    if ($page == 1 && PREVIEW_ITEMS > 0 && count($pg['slice']) > 0) {
        ...
        for ($i = 0; $i < $preview_count; $i++) {
            ...
            $preview_msg .= "   ⭐ " . ($movie['quality'] ?? 'Unknown') . " | 🗣️ " . ($movie['language'] ?? 'Hindi') . "\n\n";
        }
...
if ($update) {
    ...
    if (isset($update['callback_query'])) {
        ...
        elseif (strpos($data, 'admin_') === 0) {
            ...
            answerCallbackQuery($query['id'], "✅ Processed");
        }
    }
...
function show_latest_movies($chat_id, $limit = 10) {
    ...
    foreach ($latest as $movie) {
        ...
        $message .= "   📅 " . date('d-m-Y', strtotime($movie['added_date'])) . "\n\n";
        $i++;
    }
...
function admin_create_backup($chat_id) {
    ...
    if (copy(DB_FILE, $backup_file)) {
        ...
        $message .= "📁 <b>File:</b> backup_$timestamp.db\n";
        $message .= "💾 <b>Size:</b> $size KB\n";
        $message .= "📅 <b>Time:</b> " . date('H:i:s');
        
        sendMessage($chat_id, $message, null, 'HTML');
        admin_log("Backup created: $backup_file");
    } else {
        sendMessage($chat_id, "❌ Failed to create backup.");
    }
}
...
if (!isset($update) || !$update) {
    echo "<h1>🎬 Entertainment Tadka Bot v4.0</h1>";
