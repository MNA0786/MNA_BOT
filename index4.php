...
define('CHANNEL_6_ID', getenv('CHANNEL_6_ID') ?: '-1003614546520');  // Forwarded From Any Channel
define('GROUP_ID', getenv('GROUP_ID') ?: '-1003083386043');  // @EntertainmentTadka7860

// Legacy support (existing code ke liye)
define('CHANNEL_ID', CHANNEL_1_ID);  // Main movies channel
define('BACKUP_CHANNEL_ID', CHANNEL_2_ID);  // Backup channel
define('BACKUP_CHANNEL_USERNAME', '@ETBackup');  // Backup channel username
define('THEATER_CHANNEL_ID', CHANNEL_3_ID);  // Theater channel ID for forwarding
define('THEATER_CHANNEL', '@threater_print_movies');  // Theater channel username
define('REQUEST_CHANNEL', '@EntertainmentTadka7860');  // Request channel
define('MAIN_CHANNEL', '@EntertainmentTadka786');  // Main channel
...
// ==============================
// DELAY TYPING CONSTANTS - NEW FEATURE
// ==============================
define('TYPING_DELAY_SECONDS', 2);       // Typing delay in seconds
define('TYPING_ENABLED', true);          // Enable/disable typing indicator

// ==============================
// CHANNEL MAPPING - NEW
// ==============================
$channel_mapping = [
    CHANNEL_1_ID => '@EntertainmentTadka786',
    CHANNEL_2_ID => '@ETBackup',
    CHANNEL_3_ID => '@threater_print_movies',
    CHANNEL_4_ID => '@BackupChannel2',
    CHANNEL_5_ID => '@PrivateChannel',
    CHANNEL_6_ID => '@AnyChannel',
    GROUP_ID => '@EntertainmentTadka7860'
];

// Reverse mapping for username to ID
$channel_username_to_id = [
    '@entertainmenttadka786' => CHANNEL_1_ID,
    '@etbackup' => CHANNEL_2_ID,
    '@threater_print_movies' => CHANNEL_3_ID,
    '@backupchannel2' => CHANNEL_4_ID,
    '@privatechannel' => CHANNEL_5_ID,
    '@anychannel' => CHANNEL_6_ID,
    '@entertainmenttadka7860' => GROUP_ID,
    // Without @ prefix
    'entertainmenttadka786' => CHANNEL_1_ID,
    'etbackup' => CHANNEL_2_ID,
    'threater_print_movies' => CHANNEL_3_ID,
...
// ==============================
// DELAY TYPING FUNCTION - NEW FEATURE
// ==============================
function sendTypingAction($chat_id) {
    // Typing indicator bhejta hai
    if (TYPING_ENABLED) {
        $data = [
            'chat_id' => $chat_id,
            'action' => 'typing'
...
// ==============================
// MULTI-CHANNEL HELPER FUNCTIONS - NEW
// ==============================
function get_channel_id_from_username($username) {
    // Username se channel ID return karta hai
    global $channel_username_to_id;
    ...
    return $channel_username_to_id[$username_lower] ?? CHANNEL_1_ID;
}

function get_channel_username_from_id($channel_id) {
    // Channel ID se username return karta hai
    global $channel_mapping;
    
    return $channel_mapping[$channel_id] ?? '@UnknownChannel';
}

function is_valid_channel($channel_id) {
    // Check karta hai channel valid hai ya nahi
    $valid_channels = [CHANNEL_1_ID, CHANNEL_2_ID, CHANNEL_3_ID, CHANNEL_4_ID, CHANNEL_5_ID, CHANNEL_6_ID, GROUP_ID];
    return in_array($channel_id, $valid_channels);
}

// ==============================
// FILE INITIALIZATION FUNCTION - UPDATED FOR NEW CSV FORMAT
// ==============================
function initialize_files() {
    // Sab required files create karta hai agar nahi hain toh
    $files = [
        CSV_FILE => "movie-name,message_id,channel_username\n",  // CSV header updated
...
function load_and_clean_csv($filename = CSV_FILE) {
    ...
    if (!file_exists($filename)) {
        file_put_contents($filename, "movie-name,message_id,channel_username\n");
        return [];
    }
    ...
    if ($handle !== FALSE) {
        ...
        while (($row = fgetcsv($handle)) !== FALSE) {
            if (count($row) >= 2 && (!empty(trim($row[0])))) {
                ...
                $channel_username = isset($row[2]) ? trim($row[2]) : '@EntertainmentTadka786';
                
                // Channel ID extract karo username se
                $channel_id = get_channel_id_from_username($channel_username);
                
                // Default values extract karo (backward compatibility)
                $quality = 'Unknown';
                $size = 'Unknown';
                $language = 'Hindi';
                $date = date('d-m-Y');
                
                // Old format se data extract karo (backward compatibility)
                if (isset($row[3]) && !empty($row[3])) $date = $row[3];
                if (isset($row[4]) && !empty($row[4])) $quality = $row[4];
                if (isset($row[5]) && !empty($row[5])) $size = $row[5];
                if (isset($row[6]) && !empty($row[6])) $language = $row[6];
                
                // Channel type determine karo based on username
                $channel_type = 'main';
                if (strpos(strtolower($channel_username), 'theater') !== false || 
                    strpos(strtolower($channel_username), 'threater') !== false) {
                    $channel_type = 'theater';
                } elseif (strpos(strtolower($channel_username), 'backup') !== false) {
                    $channel_type = 'backup';
                }
    ...
    // CSV clean karo aur rewrite karo (NEW FORMAT mein)
    $handle = fopen($filename, "w");
    fputcsv($handle, array('movie-name','message_id','channel_username'));
    foreach ($data as $row) {
        fputcsv($handle, [
            $row['movie_name'], 
            $row['message_id_raw'], 
...
function sendMessage($chat_id, $text, $reply_markup = null, $parse_mode = null) {
    ...
    sendTypingAction($chat_id);  // Typing delay add karo
...
// ==============================
// /ADDMOVIE COMMAND FUNCTION - NEW
// ==============================
function handle_addmovie($chat_id, $user_id, $params) {
    // /addmovie command handle karta hai
    if ($chat_id != ADMIN_ID) {
    ...
    if (count($params) < 2) {
        sendMessage($chat_id, "❌ Usage: <code>/addmovie movie_name message_id [channel_username]</code>\n\nExample: <code>/addmovie Avengers 1234 @EntertainmentTadka786</code>", null, 'HTML');
        return;
    }
    
    $movie_name = $params[0];
    $message_id = $params[1];
    $channel_username = isset($params[2]) ? $params[2] : '@EntertainmentTadka786';
    ...
    $channel_id = get_channel_id_from_username($channel_username);
    if (!$channel_id) {
        sendMessage($chat_id, "❌ Invalid channel username. Use:\n@EntertainmentTadka786\n@ETBackup\n@threater_print_movies\n@BackupChannel2\n@PrivateChannel\n@AnyChannel\n@EntertainmentTadka7860");
        return;
    }
    
    // Add movie to CSV
    $result = append_movie_new_format($movie_name, $message_id, $channel_username);
    
    if ($result) {
        sendMessage($chat_id, "✅ Movie added successfully!\n\n🎬 Movie: <b>$movie_name</b>\n📝 Message ID: <code>$message_id</code>\n📢 Channel: $channel_username", null, 'HTML');
        bot_log("Movie added via /addmovie: $movie_name, ID: $message_id, Channel: $channel_username");
    } else {
        sendMessage($chat_id, "❌ Failed to add movie. Check logs.");
    }
}

function append_movie_new_format($movie_name, $message_id, $channel_username = '@EntertainmentTadka786') {
    // New format mein movie add karta hai
    global $movie_messages, $movie_cache;
    
    if (empty(trim($movie_name))) return false;
    
    // CSV mein entry add karo
    ...
    bot_log("Movie added (new format): $movie_name with ID $message_id to $channel_username");
    return true;
}
...
function deliver_item_to_chat($chat_id, $item, $source_type = null) {
    // Movie user ko deliver karta hai - MULTI-CHANNEL SUPPORT
    
    // Determine source channel
    $source_channel = CHANNEL_1_ID;  // Default
    
    if (isset($item['channel_id']) && is_valid_channel($item['channel_id'])) {
        $source_channel = $item['channel_id'];
        $channel_username = get_channel_username_from_id($source_channel);
    } elseif ($source_type == 'theater' && is_valid_channel(THEATER_CHANNEL_ID)) {
        $source_channel = THEATER_CHANNEL_ID;
        $channel_username = THEATER_CHANNEL;
    } else {
        $source_channel = CHANNEL_1_ID;
    ...
    $text .= "📢 Channel: " . htmlspecialchars($item['channel_username'] ?? $channel_username) . "\n";
...
function smart_search($query) {
    ...
    foreach ($movie_messages as $movie => $entries) {
        ...
        foreach ($entries as $entry) {
            $channel_username_lower = strtolower($entry['channel_username'] ?? '');
            if ($is_theater_search && 
                (strpos($channel_username_lower, 'theater') !== false || 
                 strpos($channel_username_lower, 'threater') !== false)) {
                $score += 20;  // Theater search ko theater movies ka bonus
            } elseif (!$is_theater_search && 
                     strpos($channel_username_lower, 'entertainmenttadka786') !== false) {
        ...
        if ($score > 0) {
            ...
            // Check channels
            foreach ($entries as $entry) {
                $channel_lower = strtolower($entry['channel_username'] ?? '');
                if (strpos($channel_lower, 'theater') !== false || strpos($channel_lower, 'threater') !== false) {
                    $results[$movie]['has_theater'] = true;
                }
                if (strpos($channel_lower, 'entertainmenttadka786') !== false && 
                    strpos($channel_lower, 'entertainmenttadka7860') === false) {
                    $results[$movie]['has_main'] = true;
                }
            }
        }
    }
...
function send_multilingual_response($chat_id, $message_type, $language) {
    ...
    sendTypingAction($chat_id);  // Typing delay
...
function totalupload_controller($chat_id, $page = 1, $filters = [], $session_id = null) {
    ...
    if ($page == 1 && PREVIEW_ITEMS > 0 && count($pg['slice']) > 0) {
        ...
        for ($i = 0; $i < $preview_count; $i++) {
            ...
            $channel_display = str_replace('@', '', $channel_username);
            $channel_display = substr($channel_display, 0, 15);
            
            $preview_msg .= ($i + 1) . ". <b>" . htmlspecialchars($movie['movie_name']) . "</b>\n";
            $preview_msg .= "   📢 " . $channel_display . " | ⭐ " . ($movie['quality'] ?? 'Unknown') . "\n\n";
        }
    ...
    foreach ($pg['slice'] as $movie) {
        ...
        $title .= "<b>{$i}.</b> 📢 {$channel_display} - {$movie_name}\n";
    ...
    foreach ($movies as $movie) {
        ...
        foreach ($filters as $key => $value) {
            switch ($key) {
                ...
                case 'channel':
                    $channel_lower = strtolower($movie['channel_username'] ?? '');
                    if (strpos($channel_lower, $value) === false) {
                        $pass = false;
                    }
                    break;
            }
...
// ==============================
// CHANNEL MANAGEMENT FUNCTIONS - UPDATED FOR MULTI-CHANNEL
// ==============================
function show_channel_info($chat_id) {
    ...
    $message .= "🔒 <b>Private Channel:</b> @PrivateChannel\n";
    $message .= "• Exclusive content\n";
    $message .= "• Premium movies\n";
    $message .= "• Special access\n\n";
    
    $message .= "🔀 <b>Any Channel:</b> @AnyChannel\n";
    $message .= "• Forwarded content\n";
    $message .= "• Multi-source\n";
    $message .= "• Various sources\n\n";
    ...
    $keyboard = [
        'inline_keyboard' => [
            ...
            [
                ['text' => '🔒 Private', 'url' => 'https://t.me/PrivateChannel'],
                ['text' => '🔀 Any Channel', 'url' => 'https://t.me/AnyChannel']
            ]
        ]
    ];
...
function show_theater_channel_info($chat_id) {
    ...
    $message .= "1. Join @threater_print_movies\n";
...
function show_backup_channel_info($chat_id) {
    ...
    $keyboard = [
        'inline_keyboard' => [
            [
                ['text' => '🔒 @ETBackup', 'url' => 'https://t.me/ETBackup'],
                ['text' => '📊 Backup Status', 'callback_data' => 'backup_status']
            ]
        ]
    ];
...
function show_latest_movies($chat_id, $limit = 10) {
    ...
    foreach($movies as $i => $row){
        $text_msg .= ($start+$i+1).". <b>".htmlspecialchars($row[0])."</b> | ".htmlspecialchars($row[2] ?? 'Unknown')."\n";
    }
...
function show_trending_movies($chat_id) {
    ...
    foreach (array_slice($trending_movies, 0, 10) as $movie) {
        ...
        $message .= "$i. 📢 $channel_display - <b>" . htmlspecialchars($movie['movie_name']) . "</b>\n";
...
function show_movies_by_quality($chat_id, $quality) {
    ...
    foreach (array_slice($filtered_movies, 0, 10) as $movie) {
        $channel_username = $movie['channel_username'] ?? '@Unknown';
        $channel_display = str_replace('@', '', $channel_username);
        $channel_display = substr($channel_display, 0, 10);
        
        $message .= "$i. 📢 $channel_display - " . htmlspecialchars($movie['movie_name']) . "\n";
        $i++;
    }
...
function show_csv_data($chat_id, $show_all = false) {
    ...
    foreach ($movies as $movie) {
        ...
        $channel_username = $movie[2] ?? '@EntertainmentTadka786';
        $channel_display = str_replace('@', '', $channel_username);
        
        $message .= "$i. 📢 $channel_display - " . htmlspecialchars($movie_name) . "\n";
...
function check_date($chat_id) {
    ...
    if ($h !== FALSE) {
        ...
        while (($r = fgetcsv($h)) !== FALSE) {
            // Old format compatibility - check for date in different positions
            $d = '';
            if (count($r) >= 8 && !empty($r[2]) && strpos($r[2], '-') !== false) {
                // Old format: position 2
                $d = $r[2];
            } elseif (count($r) >= 3 && !empty($r[2]) && strpos($r[2], '-') !== false) {
                // New format with date in position 2 (if exists)
                $d = $r[2];
            }
            
            if ($d && !isset($date_counts[$d])) $date_counts[$d] = 0;
            if ($d) $date_counts[$d]++;
        }
        fclose($h);
    }
...
function test_csv($chat_id) {
    ...
    if ($h !== FALSE) {
        ...
        while (($r = fgetcsv($h)) !== FALSE) {
            if (count($r) >= 1) {
                $channel_username = isset($r[2]) ? $r[2] : '@EntertainmentTadka786';
                $channel_display = str_replace('@', '', $channel_username);
                $line = "$i. 📢 $channel_display - {$r[0]} | ID: {$r[1]}";
                $line .= "\n";
...
function show_bot_info($chat_id) {
    ...
    $message .= "🎯 <b>Features:</b>\n";
    $message .= "• Multi-channel support (6 channels)\n";
    $message .= "• Smart movie search\n";
    $message .= "• Delay typing indicators\n";
    ...
    $message .= "• Support: @EntertainmentTadka7860\n";
    $message .= "• Theater: @threater_print_movies\n";
    $message .= "• Backup: @ETBackup\n";
    $message .= "• Private: @PrivateChannel\n";
    $message .= "• Any: @AnyChannel";
...
function show_support_info($chat_id) {
    ...
    $message .= "📢 <b>Support Channel:</b> @EntertainmentTadka7860\n";
...
function show_donate_info($chat_id) {
    ...
    $message .= "💌 <b>Contact for other methods:</b> @EntertainmentTadka7860";
...
function show_version_info($chat_id) {
    ...
    $message .= "• Multi-channel support (6 channels)\n";
    $message .= "• New CSV format: movie-name,message_id,channel_username\n";
    $message .= "• /addmovie command for admin\n";
    $message .= "• Delay typing indicators\n";
    $message .= "• Enhanced search across all channels\n";
...
// ==============================
// MOVIE APPEND FUNCTION WITH AUTO-NOTIFICATION - UPDATED FOR NEW FORMAT
// ==============================
function append_movie($movie_name, $message_id_raw, $date = null, $video_path = '', $quality = 'Unknown', $size = 'Unknown', $language = 'Hindi', $channel_type = 'main') {
    // Movie database mein add karta hai - OLD FORMAT (for backward compatibility)
    ...
    // Determine channel username based on channel_type
    $channel_username = '@EntertainmentTadka786';  // Default
    if ($channel_type == 'theater') {
        $channel_username = '@threater_print_movies';
    } elseif ($channel_type == 'backup') {
        $channel_username = '@ETBackup';
    }
    
    // New format mein add karo
...
function handle_command($chat_id, $user_id, $command, $params = []) {
    ...
    switch ($command) {
        ...
        case '/start':
            ...
            $welcome .= "🔒 Private: @PrivateChannel\n";
            $welcome .= "🔀 Any: @AnyChannel\n\n";
        ...
        case '/commands':
            ...
            $help .= "🔒 Private: @PrivateChannel - Exclusive content\n";
            $help .= "🔀 Any: @AnyChannel - Forwarded content\n\n";
            ...
            $help .= "• <code>/addmovie name id channel</code> - Add movie\n";
            $help .= "• <code>/broadcast message</code> - Send to all\n";
            $help .= "• <code>/backup</code> - Create backup\n";
            $help .= "• <code>/stats</code> - View statistics\n\n";
        ...
        case '/browse':
            ...
            $text_msg = "";
            foreach($movies as $i => $row){
                $channel = isset($row[2]) ? $row[2] : '@EntertainmentTadka786';
                $channel_display = str_replace('@', '', $channel);
                $channel_display = substr($channel_display, 0, 15);
                
                $text_msg .= ($start+$i+1) . ". 📢 $channel_display - <b>".htmlspecialchars($row[0])."</b>\n";
            }
            $text_msg .= "\nPage $page of $totalPages";
...
// ==============================
// MAIN UPDATE PROCESSING - UPDATED FOR MULTI-CHANNEL
// ==============================
...
if ($update) {
    ...
    // Channel post handling - UPDATED FOR MULTI-CHANNEL
    if (isset($update['channel_post'])) {
        ...
        if ($chat_id == CHANNEL_1_ID) {
            $channel_username = '@EntertainmentTadka786';
            $channel_type = 'main';
            ...
            $channel_username = '@BackupChannel2';
            $channel_type = 'backup';
        } elseif ($chat_id == CHANNEL_5_ID) {
            $channel_username = '@PrivateChannel';
            $channel_type = 'private';
        } elseif ($chat_id == CHANNEL_6_ID) {
            $channel_username = '@AnyChannel';
            $channel_type = 'any';
        } elseif ($chat_id == GROUP_ID) {
            $channel_username = '@EntertainmentTadka7860';
            $channel_type = 'group';
        } else {
        ...
        if (!empty(trim($text))) {
            // New format mein add karo
            $entry = [$text, $message_id, $channel_username];
            ...
            update_stats('total_movies', 1);
            bot_log("Auto-added from $channel_username: $text, ID: $message_id");
        }
    }
    ...
    if (isset($update['callback_query'])) {
        ...
        elseif (strpos($data, 'tu_info_') === 0) {
            ...
            foreach ($pg['slice'] as $index => $movie) {
                ...
                $info .= ($index + 1) . ". 📢 $channel_display - {$movie['movie_name']} [{$movie['quality']}]\n";
            }
        ...
        elseif (strpos($data, 'prev_') === 0) {
            ...
            for ($i = 0; $i < $limit; $i++) {
                ...
                $preview_msg .= ($i + 1) . ". 📢 $channel_display - <b>" . htmlspecialchars($movie['movie_name']) . "</b>\n";
...
if (isset($_GET['test_save'])) {
    ...
    function manual_save_to_csv($movie_name, $message_id, $channel_username = '@EntertainmentTadka786') {
        $entry = [$movie_name, $message_id, $channel_username];
        $handle = fopen(CSV_FILE, "a");
        if ($handle !== FALSE) {
            fputcsv($handle, $entry);
    ...
    // Test movies save karo for all channels
    manual_save_to_csv("Metro In Dino (2025)", 1924, "@EntertainmentTadka786");
    manual_save_to_csv("Metro In Dino 2025 WebRip 480p", 1925, "@EntertainmentTadka786");
    manual_save_to_csv("Metro In Dino (2025) Hindi 720p", 1926, "@EntertainmentTadka786");
    manual_save_to_csv("Animal (2023) Hindi 1080p", 1927, "@EntertainmentTadka786");
    manual_save_to_csv("Avengers Endgame (2019) English", 1928, "@ETBackup");
    manual_save_to_csv("KGF Chapter 2 (2022) Theater Print", 1929, "@threater_print_movies");
    manual_save_to_csv("Pushpa 2 The Rule (2024) Theater", 1930, "@threater_print_movies");
    manual_save_to_csv("Private Movie 1", 1931, "@PrivateChannel");
    manual_save_to_csv("Any Channel Movie", 1932, "@AnyChannel");
    
    echo "✅ All 9 movies manually save ho gayi! (Multi-channel support)<br>";
...
if (php_sapi_name() === 'cli' || isset($_GET['setwebhook'])) {
    ...
    if ($bot_info && isset($bot_info['ok']) && $bot_info['ok']) {
        ...
        echo "<p>Version: 3.0.0 (Multi-channel)</p>";
    }
    
    echo "<h3>Multi-Channel Setup</h3>";
    echo "<p>Channel 1 (Main): " . CHANNEL_1_ID . " - @EntertainmentTadka786</p>";
    echo "<p>Channel 2 (Backup): " . CHANNEL_2_ID . " - @ETBackup</p>";
    echo "<p>Channel 3 (Theater): " . CHANNEL_3_ID . " - @threater_print_movies</p>";
    echo "<p>Channel 4 (Backup2): " . CHANNEL_4_ID . " - @BackupChannel2</p>";
    echo "<p>Channel 5 (Private): " . CHANNEL_5_ID . " - @PrivateChannel</p>";
    echo "<p>Channel 6 (Any): " . CHANNEL_6_ID . " - @AnyChannel</p>";
    echo "<p>Group (Requests): " . GROUP_ID . " - @EntertainmentTadka7860</p>";
    ...
    echo "<p>Typing Delay: " . TYPING_DELAY_SECONDS . " seconds</p>";
...
if (!isset($update) || !$update) {
    ...
    echo "<p><strong>Multi-Channel Support:</strong> ✅ Enabled</p>";
    echo "<p><strong>Typing Delay:</strong> " . TYPING_DELAY_SECONDS . " seconds</p>";
    echo "<p><strong>CSV Format:</strong> movie-name,message_id,channel_username</p>";
    ...
    echo "<h3>🚀 Multi-Channel Setup</h3>";
    echo "<ul>";
    echo "<li>🍿 <strong>Main:</strong> @EntertainmentTadka786 (" . CHANNEL_1_ID . ")</li>";
    echo "<li>🔒 <strong>Backup:</strong> @ETBackup (" . CHANNEL_2_ID . ")</li>";
    echo "<li>🎭 <strong>Theater:</strong> @threater_print_movies (" . CHANNEL_3_ID . ")</li>";
    echo "<li>🔒 <strong>Backup2:</strong> @BackupChannel2 (" . CHANNEL_4_ID . ")</li>";
    echo "<li>🔒 <strong>Private:</strong> @PrivateChannel (" . CHANNEL_5_ID . ")</li>";
    echo "<li>🔀 <strong>Any:</strong> @AnyChannel (" . CHANNEL_6_ID . ")</li>";
    echo "<li>📥 <strong>Requests:</strong> @EntertainmentTadka7860 (" . GROUP_ID . ")</li>";
    echo "</ul>";
    ...
    echo "<li><code>/addmovie name id channel</code> - Add movie (Admin)</li>";
    ...
    echo "<h3>🎯 New Features in v3.0.0</h3>";
    echo "<ul>";
    echo "<li>✅ Multi-channel support (6 channels + 1 group)</li>";
    echo "<li>✅ New CSV format: movie-name,message_id,channel_username</li>";
    echo "<li>✅ /addmovie command for admin</li>";
    echo "<li>✅ Delay typing indicators</li>";
    echo "<li>✅ Search across all channels</li>";
    echo "<li>✅ Automatic channel detection</li>";
    echo "<li>✅ Forwarding from any channel</li>";
